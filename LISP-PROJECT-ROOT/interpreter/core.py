...
def tokenize(chars: str) -> list:
    """
    A robust lexical analyzer that handles parentheses and symbols.
    Converts a string of characters into a list of tokens.
    """
    # Uses Regex to cleanly separate parentheses and symbols
    return [
        token for token in re.findall(r'\(|\)|[^\s()]+', chars)
        if token.strip()
    ]
